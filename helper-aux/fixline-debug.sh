...
get_wine_exe_name() {
  ...
  if [ -n "$pid" ]; then
    ...
    if [ -r "/proc/$pid/cmdline" ]; then
      ...
      debug "Cannot read /proc/$pid/cmdline"
    fi
  fi
}
...
calculate_border_ids() {
  ...
  if [ -z "$main_x" ] || [ -z "$main_y" ] || [ -z "$main_w" ] || [ -z "$main_h" ]; then
    debug "Invalid main window geometry for $main_id"
    return
  fi
  
  pid=$(get_pid "$main_id")
  [ -z "$pid" ] && { debug "No PID for $main_id, falling back to WM_CLASS"; }
  ...
  for win in $candidates; do
    ...
    if [ -z "$x" ] || [ -z "$y" ] || [ -z "$w" ] || [ -z "$h" ]; then
      debug "Skipping window $win - invalid geometry"
      continue
    fi
...
xprop -root -spy _NET_ACTIVE_WINDOW | while read -r line; do
  ...
  debug "Focus change → $active_hex (class=$wm_class, name=$wm_name)"

  # Only process if wm_class matches
  if [[ -n "$wm_class" && ${APP_CONFIGS[$wm_class]+_} ]]; then
    wine_exe=$(get_wine_exe_name "$active_hex")
    debug "Window: $active_hex, Name: '$wm_name', Exe: '$wine_exe', Class: '$wm_class'"
    ...
    if [[ $is_line_app -eq 1 ]]; then
      ...
      if [ "${#edge_ids[@]}" -gt 0 ] || [ "${#corner_ids[@]}" -gt 0 ]; then
        ...
        debug "No borders found for $active_hex"
      fi
    fi
  fi
